...
if(length(gene_sets_clean_forMLM) != 0){
	if(opt$competitive == T){
		...
		Results_Comp<-foreach(i=1:length(gene_sets_clean_forMLM), .combine=rbind) %dopar% {
			if(is.na(opt$weights)){
				if(opt$covar != 'none'){
					mod2 <- relmatLmer(as.formula(paste('ZSCORE ~ TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]]', paste(opt$covar,collapse=' + '), '(0 + TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]] | FILE)', sep=' + ')), TWAS_GS_Mem_clean, relmat = list(FILE = cor_block_all))
				} else {
					mod <- relmatLmer(as.formula(paste('ZSCORE ~ TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]]', '(0 + TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]] | FILE)', sep=' + ')), TWAS_GS_Mem_clean, relmat = list(FILE = cor_block_all))
				}
			} else {
				if(opt$covar != 'none'){
					mod <- relmatLmer(as.formula(paste('ZSCORE ~ TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]]', paste(opt$covar,collapse=' + '), '(0 + TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]] | FILE)', sep=' + ')), TWAS_GS_Mem_clean, relmat = list(FILE = cor_block_all), weights=abs(TWAS_GS_Mem_clean[[opt$weights]]))
				} else {
					mod <- relmatLmer(as.formula(paste('ZSCORE ~ TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]]', '(0 + TWAS_GS_Mem_clean[,gene_sets_clean_forMLM[i]] | FILE)', sep=' + ')), TWAS_GS_Mem_clean, relmat = list(FILE = cor_block_all), weights=abs(TWAS_GS_Mem_clean[[opt$weights]]))
				}
			}
...
if(opt$self_contained == T){
	...
	Results_SelfCont<-foreach(i=1:length(gene_sets_clean), .combine=rbind) %dopar% {
		...
		if(dim(TWAS_GS_Mem_clean_selfCont)[1] > 1){
			mod <- relmatLmer(ZSCORE ~ (0 + TWAS_GS_Mem_clean_selfCont[,gene_sets_clean_forMLM[i]] | FILE), TWAS_GS_Mem_clean_selfCont, relmat = list(FILE = cor_block_all[(colnames(cor_block_all) %in% TWAS_GS_Mem_clean_selfCont$FILE),(colnames(cor_block_all) %in% TWAS_GS_Mem_clean_selfCont$FILE)]))
			coefs<-data.frame(coef(summary(mod)))
