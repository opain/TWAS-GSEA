# build_cor_matrix_signed_helper.R
#
# Signed variant of R/build_cor_matrix_helper.R. Byte-for-byte identical to the
# original EXCEPT line 40's abs() around WGCNA::cor is removed so K retains
# signed correlations. The |r|-based min_r2 / max_r2 pruning is sign-invariant
# and unchanged, so the returned matrix has the same rownames/colnames and
# sparsity pattern as the abs-K build for identical inputs.
...
build_cor_matrix <- function(genes_df, GeneX_all, cor_window, min_r2, max_r2,
	...
	cor_blocks_list <- foreach(i = unique(genes_df$Block)) %dopar% {
		...
		if(sum(genes_df$Block == i) == 1){
			...
			cor_block <- WGCNA::cor(as.matrix(GeneX_all[(names(GeneX_all) %in% genes_df$FILE[genes_df$Block == i])]), method = 'pearson')
			tmp <- cor_block
			tmp[!lower.tri(tmp)] <- 0
